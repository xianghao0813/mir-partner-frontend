"use client";

import Link from "next/link";
import { useMemo, useState } from "react";
import { useParams } from "next/navigation";

type NoticeCategory = "latest" | "events" | "updates";

type NoticePost = {
  id: number;
  category: NoticeCategory;
  title: string;
  content: string;
  author: string;
  views: number;
  createdAt: string;
};

const mockPosts: NoticePost[] = [
  {
    id: 1,
    category: "latest",
    title: "皇家藏书阁----职业技能篇",
    content: "本篇将介绍职业技能的基础机制与成长路线。",
    author: "官方",
    views: 2400,
    createdAt: "2026.01.15",
  },
  {
    id: 2,
    category: "latest",
    title: "皇家藏书阁----角色属性详解",
    content: "详细说明角色属性加成、成长方式与应用场景。",
    author: "官方",
    views: 956,
    createdAt: "2026.01.14",
  },
  {
    id: 3,
    category: "events",
    title: "米尔合伙人计划 FAQ",
    content: "关于合伙人计划的常见问题整理与说明。",
    author: "官方",
    views: 469,
    createdAt: "2026.01.06",
  },
  {
    id: 4,
    category: "updates",
    title: "公会/交易所篇更新说明",
    content: "新增交易所功能说明与公会系统改版内容。",
    author: "官方",
    views: 461,
    createdAt: "2025.12.22",
  },
  {
    id: 5,
    category: "updates",
    title: "周常任务/团本篇更新",
    content: "新增周常任务并调整团本奖励机制。",
    author: "官方",
    views: 399,
    createdAt: "2025.12.18",
  },
  {
    id: 6,
    category: "events",
    title: "春节活动预告",
    content: "春节期间将开放限时签到与特别礼包活动。",
    author: "运营团队",
    views: 1201,
    createdAt: "2026.01.18",
  },
  {
    id: 7,
    category: "latest",
    title: "新手成长指南",
    content: "帮助新玩家快速了解基础玩法与成长流程。",
    author: "官方",
    views: 888,
    createdAt: "2026.01.20",
  },
  {
    id: 8,
    category: "latest",
    title: "职业转职说明",
    content: "介绍转职条件、职业差异与注意事项。",
    author: "官方",
    views: 531,
    createdAt: "2026.01.21",
  },
  {
    id: 9,
    category: "latest",
    title: "装备强化说明",
    content: "强化概率、材料消耗与强化保护规则整理。",
    author: "GM",
    views: 333,
    createdAt: "2026.01.22",
  },
  {
    id: 10,
    category: "latest",
    title: "副本入门篇",
    content: "副本开放条件与推荐队伍搭配。",
    author: "官方",
    views: 287,
    createdAt: "2026.01.23",
  },
  {
    id: 11,
    category: "latest",
    title: "PVP 规则说明",
    content: "竞技场与野外战斗规则介绍。",
    author: "GM",
    views: 198,
    createdAt: "2026.01.24",
  },
  {
    id: 12,
    category: "events",
    title: "限时签到活动",
    content: "活动期间每日签到可领取云币与道具奖励。",
    author: "运营团队",
    views: 765,
    createdAt: "2026.01.25",
  },
];

const categoryLabelMap: Record<NoticeCategory, string> = {
  latest: "最新消息",
  events: "活动消息",
  updates: "更新消息",
};

const PAGE_SIZE = 10;

export default function NoticeCategoryPage() {
  const params = useParams();
  const category = (params.category as NoticeCategory) || "latest";

  const [searchType, setSearchType] = useState<"title_content" | "author">(
    "title_content"
  );
  const [keyword, setKeyword] = useState("");
  const [page, setPage] = useState(1);

  const filteredPosts = useMemo(() => {
    const categoryPosts = mockPosts.filter((post) => post.category === category);
    const trimmedKeyword = keyword.trim().toLowerCase();

    if (!trimmedKeyword) return categoryPosts;

    return categoryPosts.filter((post) => {
      if (searchType === "author") {
        return post.author.toLowerCase().includes(trimmedKeyword);
      }

      return (
        post.title.toLowerCase().includes(trimmedKeyword) ||
        post.content.toLowerCase().includes(trimmedKeyword)
      );
    });
  }, [category, keyword, searchType]);

  const totalPages = Math.max(1, Math.ceil(filteredPosts.length / PAGE_SIZE));

  const pagedPosts = useMemo(() => {
    const start = (page - 1) * PAGE_SIZE;
    return filteredPosts.slice(start, start + PAGE_SIZE);
  }, [filteredPosts, page]);

  const handleSearch = () => {
    setPage(1);
  };

  const handleCategoryClick = () => {
    setPage(1);
    setKeyword("");
  };

  return (
    <main
      className="hide-scrollbar"
      style={{
        height: "calc(100vh - 81px)",
        overflowY: "auto",
        background:
          "radial-gradient(circle at top, rgba(124,58,237,0.12) 0%, rgba(10,10,14,1) 32%, rgba(6,6,10,1) 100%)",
        color: "white",
        margin: "-40px",
        width: "calc(100% + 80px)",
        padding: "48px 0 80px",
      }}
    >
      <div
        style={{
          maxWidth: "1240px",
          margin: "0 auto",
          padding: "0 28px",
          boxSizing: "border-box",
        }}
      >
        <div
          style={{
            textAlign: "center",
            marginBottom: "34px",
          }}
        >
          <h1
            style={{
              fontSize: "46px",
              margin: 0,
              fontWeight: 800,
              background: "linear-gradient(90deg, #ffffff, #c084fc 55%, #8b5cf6)",
              WebkitBackgroundClip: "text",
              color: "transparent",
            }}
          >
            新闻中心
          </h1>
          <div
            style={{
              marginTop: "10px",
              letterSpacing: "0.45em",
              color: "#b7a9d8",
              fontSize: "18px",
            }}
          >
            NEWS BOARD
          </div>
          <div
            style={{
              width: "88px",
              height: "3px",
              margin: "18px auto 0",
              borderRadius: "999px",
              background: "linear-gradient(90deg, #7c3aed, #c084fc)",
              boxShadow: "0 0 18px rgba(124,58,237,0.35)",
            }}
          />
        </div>

        <div
          style={{
            background: "rgba(16,16,24,0.78)",
            border: "1px solid rgba(124,58,237,0.18)",
            borderRadius: "26px",
            padding: "28px",
            boxShadow: "0 20px 40px rgba(0,0,0,0.28)",
            backdropFilter: "blur(14px)",
          }}
        >
          <div
            style={{
              display: "flex",
              gap: "14px",
              justifyContent: "center",
              marginBottom: "26px",
              flexWrap: "wrap",
            }}
          >
            {(["latest", "events", "updates"] as NoticeCategory[]).map((item) => (
              <Link
                key={item}
                href={`/notices/${item}`}
                onClick={handleCategoryClick}
                style={{
                  textDecoration: "none",
                  padding: "10px 20px",
                  borderRadius: "999px",
                  background:
                    category === item
                      ? "linear-gradient(90deg, #7c3aed, #a855f7)"
                      : "rgba(255,255,255,0.04)",
                  color: "white",
                  fontWeight: 700,
                  border:
                    category === item
                      ? "1px solid rgba(192,132,252,0.45)"
                      : "1px solid rgba(255,255,255,0.08)",
                  boxShadow:
                    category === item
                      ? "0 0 18px rgba(124,58,237,0.24)"
                      : "none",
                }}
              >
                {categoryLabelMap[item]}
              </Link>
            ))}
          </div>

          <div
            style={{
              display: "flex",
              marginBottom: "24px",
              background: "rgba(8,8,12,0.82)",
              border: "1px solid rgba(255,255,255,0.08)",
              borderRadius: "14px",
              minHeight: "58px",
              overflow: "hidden",
            }}
          >
            <div
              style={{
                display: "flex",
                alignItems: "stretch",
                background: "rgba(255,255,255,0.02)",
                borderRight: "1px solid rgba(255,255,255,0.08)",
                flexShrink: 0,
              }}
            >
              <button
                onClick={() => setSearchType("title_content")}
                style={{
                  minWidth: "120px",
                  border: "none",
                  padding: "0 16px",
                  background:
                    searchType === "title_content"
                      ? "linear-gradient(180deg, #8b2df7, #6d28d9)"
                      : "transparent",
                  color: "white",
                  fontSize: "15px",
                  fontWeight: 700,
                  cursor: "pointer",
                }}
              >
                标题+内容
              </button>

              <button
                onClick={() => setSearchType("author")}
                style={{
                  minWidth: "88px",
                  border: "none",
                  padding: "0 16px",
                  background:
                    searchType === "author"
                      ? "linear-gradient(180deg, #8b2df7, #6d28d9)"
                      : "transparent",
                  color: "white",
                  fontSize: "15px",
                  fontWeight: 700,
                  cursor: "pointer",
                }}
              >
                作者
              </button>
            </div>

            <input
              value={keyword}
              onChange={(e) => setKeyword(e.target.value)}
              placeholder={
                searchType === "author" ? "请输入作者名称" : "请输入标题或内容关键词"
              }
              style={{
                flex: 1,
                border: "none",
                background: "transparent",
                color: "white",
                padding: "0 16px",
                fontSize: "16px",
                outline: "none",
                userSelect: "text",
                caretColor: "white",
                pointerEvents: "auto",
              }}
              onKeyDown={(e) => {
                if (e.key === "Enter") handleSearch();
              }}
            />

            <button
              onClick={handleSearch}
              style={{
                width: "84px",
                border: "none",
                background: "transparent",
                color: "#d8c5ff",
                cursor: "pointer",
                fontSize: "20px",
                flexShrink: 0,
              }}
            >
              🔍
            </button>
          </div>

          <div
            style={{
              display: "grid",
              gap: "12px",
            }}
          >
            {pagedPosts.length === 0 ? (
              <div
                style={{
                  padding: "50px 20px",
                  textAlign: "center",
                  color: "#8d8d98",
                  background: "rgba(255,255,255,0.02)",
                  borderRadius: "18px",
                  border: "1px solid rgba(255,255,255,0.05)",
                }}
              >
                搜索结果为空。
              </div>
            ) : (
              pagedPosts.map((post) => (
                <Link
                  key={post.id}
                  href={`/notices/${post.category}/${post.id}`}
                  style={{
                    display: "flex",
                    justifyContent: "space-between",
                    alignItems: "center",
                    padding: "22px 24px",
                    borderRadius: "18px",
                    textDecoration: "none",
                    color: "#f3f4f6",
                    background:
                      "linear-gradient(180deg, rgba(22,22,30,0.98) 0%, rgba(14,14,20,0.98) 100%)",
                    border: "1px solid rgba(255,255,255,0.06)",
                    boxShadow: "0 8px 18px rgba(0,0,0,0.18)",
                  }}
                >
                  <div
                    style={{
                      display: "flex",
                      gap: "18px",
                      alignItems: "flex-start",
                      minWidth: 0,
                    }}
                  >
                    <div
                      style={{
                        width: "20px",
                        height: "20px",
                        background: "linear-gradient(180deg, #8b5cf6, #7c3aed)",
                        borderRadius: "4px",
                        marginTop: "4px",
                        flexShrink: 0,
                        boxShadow: "0 0 12px rgba(124,58,237,0.28)",
                      }}
                    />
                    <div style={{ minWidth: 0 }}>
                      <div
                        style={{
                          fontSize: "18px",
                          fontWeight: 700,
                          marginBottom: "8px",
                          color: "#ffffff",
                          whiteSpace: "nowrap",
                          overflow: "hidden",
                          textOverflow: "ellipsis",
                        }}
                      >
                        {post.title}
                      </div>
                      <div
                        style={{
                          display: "flex",
                          gap: "14px",
                          color: "#9ca3af",
                          fontSize: "14px",
                          flexWrap: "wrap",
                        }}
                      >
                        <span>作者 {post.author}</span>
                        <span>浏览 {post.views.toLocaleString()}</span>
                      </div>
                    </div>
                  </div>

                  <div
                    style={{
                      color: "#a78bfa",
                      fontSize: "15px",
                      whiteSpace: "nowrap",
                      marginLeft: "20px",
                      flexShrink: 0,
                    }}
                  >
                    {post.createdAt}
                  </div>
                </Link>
              ))
            )}
          </div>

          <div
            style={{
              display: "flex",
              justifyContent: "center",
              gap: "10px",
              marginTop: "30px",
              flexWrap: "wrap",
            }}
          >
            <button
              onClick={() => setPage((prev) => Math.max(1, prev - 1))}
              disabled={page === 1}
              style={pageButtonStyle(page === 1)}
            >
              上一页
            </button>

            {Array.from({ length: totalPages }).map((_, index) => {
              const pageNumber = index + 1;
              const isActive = pageNumber === page;

              return (
                <button
                  key={pageNumber}
                  onClick={() => setPage(pageNumber)}
                  style={{
                    ...pageButtonStyle(false),
                    background: isActive
                      ? "linear-gradient(90deg, #7c3aed, #a855f7)"
                      : "rgba(255,255,255,0.04)",
                    color: "white",
                    border: isActive
                      ? "1px solid rgba(192,132,252,0.45)"
                      : "1px solid rgba(255,255,255,0.08)",
                    boxShadow: isActive
                      ? "0 0 14px rgba(124,58,237,0.22)"
                      : "none",
                  }}
                >
                  {pageNumber}
                </button>
              );
            })}

            <button
              onClick={() => setPage((prev) => Math.min(totalPages, prev + 1))}
              disabled={page === totalPages}
              style={pageButtonStyle(page === totalPages)}
            >
              下一页
            </button>
          </div>
        </div>
      </div>
    </main>
  );
}

function pageButtonStyle(disabled: boolean): React.CSSProperties {
  return {
    minWidth: "46px",
    height: "42px",
    padding: "0 14px",
    borderRadius: "10px",
    border: "1px solid rgba(255,255,255,0.08)",
    background: disabled ? "rgba(255,255,255,0.03)" : "rgba(255,255,255,0.04)",
    color: disabled ? "#6b7280" : "white",
    cursor: disabled ? "not-allowed" : "pointer",
    fontWeight: 700,
  };
}
